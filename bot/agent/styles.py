...
CONCISE_RESPONSE_GUIDE = (
    "공통 답변 지침: 짧지만 실속 있게 답하되, 매번 같은 형식으로 정리하지 말고 상황과 스타일에 맞춰 자연스럽게 답하라. "
    "항상 결론/근거/마무리 제안 같은 고정 구조를 만들 필요는 없다. 단답으로 끝내면 부족한 질문에는 이유나 예시를 자연스럽게 붙여라. "
    "장황한 서론, 과한 꾸밈말, 불필요한 반복은 줄이되 중요한 주의점과 필요한 정보는 생략하지 마라. "
    "추가로 할 일을 억지로 붙이지 말고, 사용자가 원했거나 답변 이해에 꼭 필요할 때만 제안하라. "
    ...
    "Discord 멤버를 가리킬 때는 숫자 ID만 쓰거나 이름(숫자ID)처럼 쓰지 말고, 반드시 `<@유저ID>` 형식의 유저 멘션을 사용하라. "
    "`<@닉네임>`, `<@이름>`처럼 ID 자리에 이름을 넣은 가짜 멘션은 절대 쓰지 마라. `<@...>` 안에는 실제 숫자 ID만 들어간다. "
