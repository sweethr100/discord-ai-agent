...
```env
SYSTEM_PROMPT=너는 한국어로 짧고 명확하게 답하는 Discord AI 도우미다.
```
...
AI provider가 한 번에 생성할 최대 길이는 `.env`의 `AI_MAX_TOKENS`로 조절합니다.

```env
AI_MAX_TOKENS=4096
```

긴 사용 설명서나 서버 관리 기능 목록이 중간에 끊기면 `.env`에 남아 있는 `AI_MAX_TOKENS=1024` 같은 낮은 값을 `4096` 이상으로 올린 뒤 봇을 재시작하세요. Discord 메시지 2000자 제한은 봇이 자동으로 여러 메시지로 나누어 전송합니다.
...
- 답변이 `### 서버 운영`처럼 중간에 끊기면 provider 출력 한도에 걸린 것입니다. `.env`의 `AI_MAX_TOKENS`를 `4096` 이상으로 올리고 봇을 재시작하세요.
