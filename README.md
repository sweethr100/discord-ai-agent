...
```text
/ai message:<선택> style:<선택> attachment:<선택>
```

- 모든 사용자가 사용할 수 있습니다.
- `style`은 선택값입니다.
- `style`을 넣으면 해당 요청에만 임시 적용되고, 서버 기본 스타일은 바뀌지 않습니다.
- `attachment`에 이미지 파일을 넣거나, 봇 멘션 메시지에 이미지를 첨부하면 이미지 내용을 함께 읽습니다.
- 메시지 안에 `https://...` URL을 넣으면 웹 페이지의 텍스트 또는 이미지 URL을 가져와 함께 참고합니다.
- 이미지 읽기는 현재 선택한 provider/model이 vision 입력을 지원해야 정상 동작합니다.
...
/ai message:이 스크린샷에서 에러 원인 알려줘 attachment:<이미지>
@봇 https://example.com 이 글 핵심만 요약해줘
```
...
자동 응답 채널의 모드를 바꾸려면 기존 등록을 제거한 뒤 `/autochannel add`로 다시 등록하세요.
...
/style remove name:<스타일 이름>
/style channel channel:<채널> style:<스타일|server_default>
```
...
/style channel channel:#news style:news
/style remove name:news
/style show
```
